...
import { Message, UserLastMessage, SupportMessage } from '../models/user-last-message.model';
...
export class SupportService {
  ...
  async getChats() {
    ...
    try {
      ...
      if (!isConnected) {
        console.warn('SignalR: Connection not ready, skipping getChats');
        return;
      }

      const result = await this.hubConnection.invoke<UserLastMessage[]>('GetContactedUsersOrdered');
  ...
  async GetMessagesWithUser(otheruserId: string) {
    ...
    try {
      ...
      const result = await this.hubConnection.invoke<SupportMessage[]>('GetMessagesWithUser', otheruserId);
  ...
  async markMessagesAsRead(otheruserId: string) {
    ...
    try {
      const isConnected = await this.waitForConnection();
      if (!isConnected) {
        console.warn('SignalR: Connection not ready, cannot mark messages as read');
        return;
      }
  ...
  async deleteMessage(otherUserId: string, messageId: number): Promise<boolean> {
    if (!this.isBrowser || !this.hubConnection) {
      console.warn('Cannot delete message: browser check failed or no hub connection');
      return false;
    }
    ...
    try {
      ...
      console.log('Connection ready, invoking DeleteMessage with:', { otherUserId, messageId });
      const result = await this.hubConnection.invoke<boolean>('DeleteMessage', otherUserId, messageId);
